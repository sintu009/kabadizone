...
import { IndianRupee, ArrowUpRight, ArrowDownRight, Loader2 } from 'lucide-react';
import { fetchMyWallet, fetchMyTransactions } from '../features/walletSlice';

const EarningsPage = () => {
  const { t } = useTranslation();
  const dispatch = useDispatch();
  const { balance, transactions, walletLoading } = useSelector((s) => s.scrapboyWallet);
    ...
    <div className="space-y-5">
      ...
      <div className="bg-gradient-to-br from-emerald-600 to-emerald-700 rounded-2xl p-5 text-white shadow-lg">
        ...
        <p className="text-3xl font-bold mt-1 flex items-center gap-1">
          <IndianRupee className="h-7 w-7" />{Number(balance).toLocaleString()}
        </p>
            ...
            {transactions.map((tx) => {
              ...
              return (
                <div key={tx.id} className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${isCredit ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-500'}`}>
                    {isCredit ? <ArrowDownRight className="h-5 w-5" /> : <ArrowUpRight className="h-5 w-5" />}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-gray-900 truncate">{tx.remarks || tx.transaction_type}</p>
                    <p className="text-xs text-gray-400">
                      {(tx.created_on || tx.created_at) ? new Date(tx.created_on || tx.created_at).toLocaleDateString() : '—'}
                    </p>
                  </div>
